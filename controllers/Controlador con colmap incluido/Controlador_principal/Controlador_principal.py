...
from captura import CapturaImagenes
...
#camara para colmap
captura = CapturaImagenes(camera)
foto_intervalo=0.2
ultima_foto=0.0
objetivo_fotos=200
...
while robot.step(timestep) != -1:
    ...
    if funcionando:
        ...
        if t_act - ultima_foto >= foto_intervalo:

            if captura.count_images() < objetivo_fotos:

                if captura.save_image():
